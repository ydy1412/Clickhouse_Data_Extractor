...
def preprocess_sample_data(data_extract_context, table_name, start_dttm, last_dttm, encoder_dict, data_cnt=100000):
    sample_data = data_extract_context.Extract_Sample_Log(table_name, start_dttm, last_dttm, targeting='AD',
                                                          sample_cnt=data_cnt)
    ...
    for column in encode_column_list:
        print(column)
        encoded_array = encoder_dict[column].transform(sample_data[[column]]).toarray()
        encoded_array_list.append(encoded_array)

    encoded_array_list.append(hash_trick(sample_data[['MEDIA_CATE_INFO']], 20))
    encoded_array_list.append(hash_trick(sample_data[['SCRIPT_TP_CODE']], 25))
    encoded_array_list.append(hash_trick(sample_data[['ADVER_MEDIA_HASH_KEY']]))
    encoded_array_list.append(hash_trick(sample_data[['BROWSER_CODE']], 20))
    encoded_array_list.append(hash_trick(sample_data[['OS_CODE']], 15))
...
if __name__ == '__main__' :
    ...
    for i in range(1):
        ...
        input_data, output_data = preprocess_sample_data(test_context, 'CLICK_VIEW_YN_LOG_NEW',
                                                         '20201001', '20201010', encoder_dict, 100000)
